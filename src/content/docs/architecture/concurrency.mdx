...
Forked processes copy memory, so memory usage scales with the number of pipelines. Strategies for managing this include running multiple `bin/ductwork` processes across machines or switching to the threaded model. See [Scaling](/docs/advanced/scaling) for more details.
...
See [Configuration](/docs/getting-started/configuration) for additional options.
