...
## `job_worker.max_crash`

Sets the maximum number of times a step's job can be retried after a crash before marking it as an error and halting the pipeline. Unlike [`job_worker.max_retry`](#job_workermax_retry), which governs retries when a job *raises an error*, this limit governs *crashes* — out-of-memory kills, parent process crashes, other OS-level crashes, thread crashes, and hard deploys that kill threads and processes.

The first 3 crashes are retried immediately with no back-off. After that, a tail back-off is applied between subsequent attempts before the job, and therefore the step and pipeline, is finally failed.
...
You can also set pipline and step-level max crash configuration. When using this syntax you can still set defaults at the library and pipeline-level with the `max_crash.default` and `max_crash.MyPipelineA.default` keys.
...
Configures the *average* time (in seconds) a job worker thread sleeps when it is idle — that is, when it queried for claimable job IDs and none were returned. This prevents excessive database queries while waiting for work.

The configured value is not used verbatim. Jitter of +/- 25% is applied to each sleep, so a value of `1` results in individual sleeps between 0.75 and 1.25 seconds. This spreads out polling across threads and processes to avoid a thundering herd hammering the database in lockstep.
...
Sets the number of threads created for advancing pipelines. You can scale this for high-throughput environments where pipeline advancement becomes a bottleneck.
...
You can also set per-pipeline thread counts for fine-grained control. Pipelines without explicit configuration inherit the `default` value.
...
Configures the *average* time (in seconds) a pipeline advancer thread sleeps when it is idle — that is, when it queried for claimable pipeline IDs and none were returned. The advancer queries for pipelines whose last step is completed and advances them if found.
...
Specifies which pipelines to run for the Ductwork process. Running a pipeline means starting both a pipeline advancer process and a job worker process with multiple threads. For more information on Ductwork's concurrency model, see the [Concurrency page](/docs/architecture/concurrency/).
...
## `supervisor.reaper_timeout`

Configures how long (in seconds) a child process's heartbeat can go without being updated before the supervisor considers the process unhealthy and reaps its resources. When a process is reaped, any in-progress pipeline advancements and job executions it claimed are marked as crashed so they can be picked up again. This timeout should be longer than `supervisor.polling_timeout` to give heartbeats time to register between checks. Realistically, this value should never be under 60 seconds.
