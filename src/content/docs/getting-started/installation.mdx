...
If you are connecting to multiple databases in your rails application you can configure which database to use with the `database` configuration. For more information see the [Configuration page](/docs/getting-started/configuration/). Then, you'll need to copy over the migrations that were generated to the `migration_paths` configuration for that database before migrating.

---

Once that is completed you are ready to [define pipelines](/docs/getting-started/defining-pipelines/) and [run the `ductwork` process](/docs/getting-started/running-pipelines/).
